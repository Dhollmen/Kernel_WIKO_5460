...
//extern DEFINE_MUTEX(synaptics_i2c_access);
extern struct mutex synaptics_i2c_access;

#ifdef DO_STARTUP_FW_UPDATE
static void synaptics_read_fw_version(void);
#endif
...
#ifdef DO_STARTUP_FW_UPDATE
static int gconfig_id = 0;//yixuhong 20141225 add ,save config id
#endif
...
int get_synaptics_versionid( void )
{
    int retval = 0;
...
int get_synaptics_image_versionid( void )
{     
      return   fwu_IMG_ID();	
}

//kangqin add begin
int set_synaptics_charge(bool bIsChargedIn)
{
	int retval;
	...
	f01_device_control.charger_connected = bIsChargedIn;
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev,
			...
			fwu->config_data[3]);
             ...
             CTP_DBG("%s: line=%ld,ftm_force_update=%ld \n",__func__,__LINE__,ftm_force_update);
	if (1==ftm_force_update) {
		...
		unsigned char command)
{
	...
	for (block_num = 0; block_num < block_cnt; block_num++) {
		...
		if (retval < 0) {
			dev_err(&fwu->rmi4_data->i2c_client->dev,
					"%s: Failed to write block data (block %ld)\n",
					__func__, block_num);
			return retval;
		}
		...
		if (retval < 0) {
			dev_err(&fwu->rmi4_data->i2c_client->dev,
					"%s: Failed to write command for block %ld\n",
					__func__, block_num);
			return retval;
		}
		...
		if (retval < 0) {
			dev_err(&fwu->rmi4_data->i2c_client->dev,
					"%s: Failed to wait for idle status (block %ld)\n",
					__func__, block_num);
			return retval;
		}
...
static int fwu_do_read_config(void)
{
	...
	for (block_num = 0; block_num < block_count; block_num++) {
		...
		if (retval < 0) {
			dev_err(&fwu->rmi4_data->i2c_client->dev,
					"%s: Failed to read block data (block %ld)\n",
					__func__, block_num);
			goto exit;
		}
...
int update_FW(void)
{
 /*  fwu->fwu_workqueue = create_singlethread_workqueue("fwu_workqueue");
   ...
   queue_delayed_work(fwu->fwu_workqueue,
        ...
        fw_data = FW_IMG_DEFAULT;
        synaptics_fw_updater(fw_data);
        #else
...
int fwu_IMG_ID(void)
{
	...
	if (fwu->rmi4_data->sensor_sleep) {
		printk(&fwu->rmi4_data->i2c_client->dev,
				"%s: Sensor sleeping\n",
				__func__);
		return -ENODEV;
	}
	...
	if (header.config_size) {
        ...
        retval = extract_uint_be(fwu->config_data);
        printk("==== fwu_IMG_ID=%ld====", extract_uint_be(fwu->config_data));
...
static int fwu_start_reflash(void)
{
	...
	CTP_DBG("%s: Start of reflash processxxxx1\n", __func__);

	if (fwu->ext_data_source) {
		fw_image = fwu->ext_data_source;
	} else {
		strncpy(fwu->image_name, FW_IMAGE_NAME, MAX_IMAGE_NAME_LEN);
		CTP_DBG("%s: Requesting firmware image %s\n",
				__func__, fwu->image_name);

		retval = request_firmware(&fw_entry, fwu->image_name,
				&fwu->rmi4_data->i2c_client->dev);
		if (retval != 0) {
			CTP_DBG("%s: Firmware image %s not available\n",
					__func__, fwu->image_name);
			retval = -EINVAL;
			goto exit;
		}

		CTP_DBG("%s: Firmware image size = %ld\n",
				__func__, fw_entry->size);
	...
	if (fwu->bl_version != header.bootloader_version) {
		CTP_DBG("%s: Bootloader version mismatch\n",
				__func__);
		retval = -EINVAL;
		goto exit;
	}
	...
	if (fwu->do_lockdown) {
		...
		case V6:
			...
			retval = fwu_do_lockdown();
			if (retval < 0) {
				CTP_DBG("%s: Failed to do lockdown\n",
						__func__);
			}
		default:
			break;
	...
	flash_area = fwu_go_nogo(&header);
             CTP_DBG("flash_area=%ld",flash_area);
	switch (flash_area) {
	case UI_FIRMWARE:
		retval = fwu_do_reflash();
		break;
	...
	if (retval < 0) {
		CTP_DBG("%s: Failed to do reflash\n",
				__func__);
	}
...
exit:
	...
	CTP_DBG("%s: End of reflash process\n", __func__);
...
//	mutex_unlock(&synaptics_i2c_access);
    //@xuchunsheng added for refresh vertion number when complete the update firmware
    synaptics_read_fw_version();
	return retval;
}
...
#ifdef DO_STARTUP_FW_UPDATE
//yixuhong 20141126 add for query tp information
static void synaptics_read_fw_version(void)
{
    int ret;
    unsigned char config_id[4]={0};
	...
	printk("%s:config_id= %x%x%x%x \n",__func__,config_id[1],config_id[1],config_id[2],config_id[3]);

	gconfig_id = be_to_uint(config_id);    
}
...
int synaptics_get_fw_version(void)
{
        int ret,reth,retl;
        ret = reth = retl =0;
        reth = (gconfig_id&0xFF00)>>8; 
        retl = gconfig_id&0xFF;
        reth = reth - 0x30;
        retl = retl - 0x30;
        ret = reth*10 + retl;       
        printk("%s:gconfig_id=%x,ret=%x",__func__,gconfig_id,ret);
        //return gconfig_id;
        return ret;
}
