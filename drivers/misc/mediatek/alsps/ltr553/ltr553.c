...
static int set_low_temperature_threshold_value(struct ltr553_priv *obj,int ps)
{
	...
	{
//BEGIN <EGAFM-306> <the smart lcd backlight shutdown doesn't work> <> panzaoyan
if(ps>0x500)
	{

			atomic_set(&ltr553_obj->ps_thd_val_high, 0x520);
    		       atomic_set(&ltr553_obj->ps_thd_val_low,  0x4a0);
	        	return ps;
	}


//END <EGAFM-306> <the smart lcd backlight shutdown doesn't work> <> panzaoyan
		if(g_ps_base_value+50<ps)
		{	
...
int ltr553_init_device(struct i2c_client *client)
{
    ...
    hwmsen_write_byte(client, 0x9e, 0x00);              ///2 consecutive data outside range to interrupt joven modify 0x40 ->0x00
